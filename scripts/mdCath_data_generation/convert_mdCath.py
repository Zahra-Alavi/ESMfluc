...
def convert_to_files(
    fn, basename=None, temp_list=[320, 348, 379, 413, 450], replica_list=[0, 1, 2, 3, 4], output_dir=None
):
    """
    ...
    output_dir : str, optional
        The directory where output files will be saved. If None, files are saved in the current working directory. Defaults to None.
    ...
    output_folder = output_dir if output_dir else "."
    pdbpath = os.path.join(output_folder, f"{basename}.pdb")
    ...
    for temp in temp_list:
        for replica in replica_list:
            xtcpath = os.path.join(output_folder, f"{basename}_{temp}_{replica}.xtc")
...
def main():
    ...
    convert_to_files(args.fn, args.basename, args.temp_list, args.replica_list, args.output_dir)
