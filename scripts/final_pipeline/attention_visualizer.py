...
def visualize_attention(attention_weights, tokens: list, seq_id: str, record: dict, annotation_features: list):
    ...
    # attention heatmap with percentile-based color scaling
    # Use 95th percentile to avoid saturation from extreme values
    vmin = 0
    vmax = np.percentile(attention_weights, 95)
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            zmax=vmax,
            colorbar=dict(
                title="Attention<br>(95th percentile)",
                x=1.15,
                y=0.45,
                len=0.9
            ),
...
def pheatmap_visualizer(attention_weights, tokens: list, seq_id: str, record: dict, annotation_features: list):
    ...
    # Use percentile-based color scaling to avoid saturation from outliers
    vmin = 0
    vmax = np.percentile(attention_weights, 95)
