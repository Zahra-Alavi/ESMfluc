...
def visualize_attention(model, tokenizer, seq, seq_id):
    ...
    # Convert token IDs to amino acid tokens
    token_ids = input_ids[0].tolist()  # shape = [seq_len] for batch_size=1
    ...
    # 5) Create a Plotly Heatmap
    fig = go.Figure(
        data=go.Heatmap(
            z=attn_weights,
            text=hover_text,       # custom text for hover
            hoverinfo='text',      # display that text on hover
            colorscale='Viridis',
